...
import styles from './Download1-19-1.module.css';
...
const Download1191 = () => {
  ...
  // Все доступные моды (полный массив из PreDownload1-19-1)
  const allMods: Mod[] = [
    {
      id: 'cloth-config',
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedMods.length; i++) {
        ...
        try {
          // Пытаемся загрузить файл мода из папки assets/Download1-19-1/files
          const response = await fetch(`/assets/Download1-19-1/files/${mod.id}.jar`);
      ...
      try {
        const optionsResponse = await fetch('/assets/Download1-19-1/files/options.txt');
      ...
      try {
        const configZipResponse = await fetch('/assets/Download1-19-1/files/config.zip');
      ...
      link.href = url;
      link.download = 'optisodium1-19-1.zip';
            ...
            {!downloadComplete ? (
              <>
                <h2 className={styles.title}>
                  Выбрано {selectedMods.length} модов для Minecraft Java Edition 1.19.1
                </h2>
