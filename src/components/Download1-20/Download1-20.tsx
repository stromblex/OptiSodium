...
import styles from './Download1-20.module.css';
...
const Download120 = () => {
  ...
  // Все доступные моды (полный массив из PreDownload1-20)
  const allMods: Mod[] = [
    {
      id: 'cloth-config',
    ...
    try {
      ...
      for (let i = 0; i < selectedMods.length; i++) {
        ...
        try {
          ...
          const response = await fetch(`/assets/Download1-20/files/${mod.id}.jar`);
      ...
      try {
        const optionsResponse = await fetch('/assets/Download1-20/files/options.txt');
      ...
      try {
        const configZipResponse = await fetch('/assets/Download1-20/files/config.zip');
      ...
      link.href = url;
      link.download = 'optisodium1-20.zip';
        ...
        <div className={styles.contentPanel}>
          ...
          <div className={styles.header}>
            <div className={styles.logo} onClick={handleLogoClick} style={{ cursor: 'pointer' }}>
              ...
              />
              <span className={styles.logoText}>OptiSodium 1.20</span>
            </div>
          </div>
            ...
            {!downloadComplete ? (
              <>
                <h2 className={styles.title}>
                  Выбрано {selectedMods.length} модов для Minecraft Java Edition 1.20
                </h2>
