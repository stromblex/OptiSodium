...
import styles from './Download1-19.module.css';
...
const Download119 = () => {
  ...
  // Все доступные моды (полный массив из PreDownload1-19)
  const allMods: Mod[] = [
    {
      id: 'cloth-config',
    ...
    },
    {
      ...
      name: 'FabricSkyBoxes',
      size: 0.15,
      description: 'Добавляет поддержку пользовательских текстур неба из OptiFine. Позволяет использовать ресурс-паки с красивыми небосводами.',
      url: 'https://modrinth.com/mod/nuit-interop',
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      let progress = 0;
      const totalItems = selectedMods.length + 2; // +2 для config папки и options.txt
      let currentItem = 0;
      ...
      for (let i = 0; i < selectedMods.length; i++) {
        ...
        try {
          // Пытаемся загрузить файл мода из папки assets/Download1-19/files
          const response = await fetch(`/assets/Download1-19/files/${mod.id}.jar`);
      ...
      try {
        const optionsResponse = await fetch('/assets/Download1-19/files/options.txt');
      ...
      // Просто добавляем config.zip в корень архива
      try {
        const configZipResponse = await fetch('/assets/Download1-19/files/config.zip');
        if (configZipResponse.ok) {
          ...
          console.warn('❌ config.zip не найден, архив будет без конфига');
        }
      } catch (error) {
      ...
      link.href = url;
      link.download = 'optisodium1-19.zip';
        ...
        <div className={styles.contentPanel}>
          ...
          <div className={styles.header}>
            <div className={styles.logo} onClick={handleLogoClick} style={{ cursor: 'pointer' }}>
              ...
              />
              <span className={styles.logoText}>OptiSodium 1.19</span>
            </div>
          </div>
            ...
            {!downloadComplete ? (
              <>
                <h2 className={styles.title}>
                  Выбрано {selectedMods.length} модов для Minecraft Java Edition 1.19
                </h2>
